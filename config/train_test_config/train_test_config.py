...
def print_train_loss_XNetv2(train_loss_sup_1, train_loss_sup_2, 
                            # train_loss_sup_3, 
                            train_loss_unsup, train_loss, num_batches, print_num, print_num_minus): # train_loss_sup_3, 
    ...
    print('-' * print_num)
    return train_epoch_loss_sup1, train_epoch_loss_sup2, train_epoch_loss_unsup, train_epoch_loss # train_epoch_loss_sup3, 
...
def print_val_eval_sup(num_classes, score_list_val, mask_list_val, print_num):
    if num_classes == 2:
        # print(score_list_val.shape)
        # print(mask_list_val.shape)
    ...
    if num_classes == 2:
        if best_list[1] < eval_list[1]:
            ...
            for i in range(len(name_list_val)):
                ...
                color_results.save(os.path.join(path_seg_results, name_list_val[i].replace('.jpg', '.png')))
...
def print_test_eval(num_classes, score_list_test, mask_list_test, print_num):
    if num_classes == 2:
        ...
        # print('| Test  asd: {:.4f}'.format(eval_list[3]).ljust(print_num, ' '), '|')
        # print('| Test  95hd: {:.4f}'.format(eval_list[4]).ljust(print_num, ' '), '|')
    else:
...
def save_test_2d(num_classes, score_list_test, name_list_test, threshold, path_seg_results, palette):
    ...
    if num_classes == 2:
        ...
        for i in range(len(name_list_test)):
            # print(pred_results[i].astype(np.uint8))
            color_results = Image.fromarray(pred_results[i].astype(np.uint8), mode='P')
            color_results.putpalette(palette)
