...
from config.augmentation.online_aug import data_transform_2d, data_normalize_2d, data_transform_strong_2d #, data_strong_resize_2d
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_scale', default=[0.5, 0.9])
    parser.add_argument('--val_scale', default=[0.6, 0.6])
    ...
    # data_strong_resize = data_strong_resize_2d(cfg['INPUT_SIZE'])
    # data_strong_transform = data_transform_strong_2d()
    ...
    dataset_train = imagefloder_XNetv2(
        ...
        # data_strong_transform_1=data_strong_transform['train'],
        # data_strong_resize_1=data_strong_resize['train'],
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, data in enumerate(dataloaders['train']):
            ...
            inputs_train_1 = Variable(data['H'].cuda())
            inputs_train_2 = Variable(data['L'].cuda())
            # inputs_train_3 = Variable(data['H'].cuda())
            inputs_train_3 = torch.cat((Variable(data['image'].cuda()), Variable(data['LH_'].cuda()), 
                                    #    Variable(data['LH'].cuda()), Variable(data['HL'].cuda()),
                                        # Variable(data['H'].cuda()),
            ), dim=1)
            mask_train = Variable(data['mask'].cuda())

            # inputs_train_strong_1 = Variable(data['H'].cuda())
            # inputs_train_strong_2 = Variable(data['L'].cuda())
            # print(data['image_strong'].shape)
            # inputs_train_strong_3 = torch.cat((Variable(data['image_strong'].cuda()), Variable(data['LH_'].cuda())),
            #                                   dim=1)
            # print(data['L_strong'].shape)
            # print(data['H_strong'].shape)
            # print(data['image_strong'].shape)
            # inputs_train_strong_3 = torch.cat((Variable(data['image_strong'].cuda()), Variable(data['L_strong'].cuda()),
            #                            Variable(data['LH_strong'].cuda()), Variable(data['HL_strong'].cuda()),
            #                             Variable(data['H_strong'].cuda())), dim=1)
            # # print(inputs_train_1.shape)
            # print(inputs_train_2.shape)
            # print(inputs_train_3.shape)
            # mask_train_strong = Variable(data['mask_strong'].cuda())

            optimizer.zero_grad()
            outputs_train1, outputs_train2 = model(inputs_train_1, inputs_train_2, inputs_train_3)  # , outputs_train3 
            ...
            outputs_train_softmax1 = torch.softmax(outputs_train1, dim=1)
            outputs_train_softmax2 = torch.softmax(outputs_train2, dim=1)

            loss_train_sup1 = criterion(outputs_train1, mask_train)  + F.cross_entropy(outputs_train_softmax1, mask_train)
            loss_train_sup2 = criterion(outputs_train2, mask_train)  + F.cross_entropy(outputs_train_softmax2, mask_train)
            loss_train_unsup = criterion(outputs_train1, max_train2) + criterion(outputs_train2, max_train1)  + \
                           F.cross_entropy(outputs_train_softmax1, max_train2) + F.cross_entropy(outputs_train_softmax2, max_train1)
            ...
            # outputs_train1_strong, outputs_train2_strong = model(inputs_train_strong_1, inputs_train_strong_2, inputs_train_strong_3) 
            # outputs_train_softmax1_strong = torch.softmax(outputs_train1_strong, dim=1)
            # outputs_train_softmax2_strong = torch.softmax(outputs_train2_strong, dim=1)
            # loss_train_unsup = criterion(outputs_train1_strong, mask_train) + criterion(outputs_train2_strong, mask_train) + \
            #     F.cross_entropy(outputs_train_softmax1_strong, max_train2) + F.cross_entropy(outputs_train_softmax2_strong, max_train1)
            # loss_train_unsup = loss_train_unsup * unsup_weight
            # loss_train_ = loss_train_unsup
        ...
        if count_iter % args.display_iter == 0:
            ...
            if rank == args.rank_index:
                ...
                train_epoch_loss_sup1, train_epoch_loss_sup2, train_epoch_loss_unsup, train_epoch_loss = print_train_loss_XNetv2(train_loss_sup_1, train_loss_sup_2, train_loss_unsup, train_loss, num_batches, print_num, print_num_minus) # train_epoch_loss_sup3 train_loss_sup_3, 
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(dataloaders['val']):
                    ...
                    inputs_val1 = Variable(data['H'].cuda())
                    inputs_val2 = Variable(data['L'].cuda())
                    # inputs_val3 = Variable(data['H'].cuda())
                    inputs_val3 = torch.cat((Variable(data['image'].cuda()), Variable(data['LH_'].cuda()), 
                                    #    Variable(data['LH'].cuda()), Variable(data['HL'].cuda()),
                                        # Variable(data['H'].cuda())
                                        ),dim=1)
                    ...
                    optimizer.zero_grad()
                    outputs_val1, outputs_val2, = model(inputs_val1, inputs_val2, inputs_val3) # outputs_val3 
                    torch.cuda.empty_cache()
                ...
                if rank == args.rank_index:
                    ...
                    best_val_eval_list1 = save_val_best_sup_2d(cfg['NUM_CLASSES'], best_val_eval_list1, model, score_list_val1, name_list_val, val_eval_list1, path_trained_models, path_seg_results, cfg['PALETTE'], 'XNetv2')
                    best_val_eval_list2 = save_val_best_sup_2d(cfg['NUM_CLASSES'], best_val_eval_list2, model, score_list_val2, name_list_val, val_eval_list2, path_trained_models, path_seg_results, cfg['PALETTE'], 'XNetv2')
                    torch.cuda.empty_cache()
