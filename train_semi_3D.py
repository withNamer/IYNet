...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_gamma', default=[0.7, 0.9])
    ...
    for epoch in range(args.num_epochs):
        ...
        for i in range(num_batches['train_sup']):
            ...
            img_train_unsup_4 = torch.zeros_like(img_train_unsup_1, device='cpu')
            ...
            for j in range(img_train_unsup_numpy.shape[0]):
                ...
                LH_gamma = random.uniform(args.train_gamma[0], args.train_gamma[1])
                ...
                img_train_unsup_4[j] = LH_
            img_train_unsup_2 = Variable(img_train_unsup_2.cuda())
            img_train_unsup_3 = Variable(img_train_unsup_3.cuda())
            img_train_unsup_4 = Variable(img_train_unsup_4.cuda())
            img_train_unsup_1 = torch.cat([img_train_unsup_1, img_train_unsup_4], dim=1)
            torch.cuda.empty_cache()

            optimizer1.zero_grad()
            pred_train_unsup1, pred_train_unsup2 = model1(img_train_unsup_2, img_train_unsup_3, img_train_unsup_1)

            max_train_unsup1 = torch.max(pred_train_unsup1, dim=1)[1].long()
            max_train_unsup2 = torch.max(pred_train_unsup2, dim=1)[1].long()
            outputs_train_softmax_unsup1 = torch.softmax(pred_train_unsup1, dim=1)
            outputs_train_softmax_unsup2 = torch.softmax(pred_train_unsup2, dim=1)
            # max_train_unsup3 = torch.max(pred_train_unsup3, dim=1)[1].long()

            loss_train_unsup = criterion(pred_train_unsup1, max_train_unsup2) + F.cross_entropy(outputs_train_softmax_unsup1, max_train_unsup2) + \
                               criterion(pred_train_unsup2, max_train_unsup1) + F.cross_entropy(outputs_train_softmax_unsup2, max_train_unsup1)
            ...
            for j in range(img_train_sup_numpy.shape[0]):
                ...
                img_train_sup_2[j] = L
                img_train_sup_3[j] = H
                img_train_sup_4[j] = LH_
            img_train_sup_2 = Variable(img_train_sup_2.cuda())
            img_train_sup_3 = Variable(img_train_sup_3.cuda())
            img_train_sup_4 = Variable(img_train_sup_4.cuda())
            mask_train_sup = Variable(sup_index['mask'][tio.DATA].squeeze(1).long().cuda())
            img_train_sup_1 = torch.cat([img_train_sup_1, img_train_sup_4], dim=1)
            torch.cuda.empty_cache()

            pred_train_sup1, pred_train_sup2 = model1(img_train_sup_2, img_train_sup_3, img_train_sup_1)
            ...
            outputs_train_softmax_sup1 = F.softmax(pred_train_sup1, dim=1)
            outputs_train_softmax_sup2 = F.softmax(pred_train_sup2, dim=1)
            loss_train_sup1 = criterion(pred_train_sup1, mask_train_sup) + F.cross_entropy(pred_train_sup1, mask_train_sup)
            loss_train_sup2 = criterion(pred_train_sup2, mask_train_sup) + F.cross_entropy(pred_train_sup2, mask_train_sup)
        ...
        if count_iter % args.display_iter == 0:
            ...
            if rank == args.rank_index:
                ...
                train_epoch_loss_sup_1, train_epoch_loss_sup_2, train_epoch_loss_unsup, train_epoch_loss = print_train_loss_XNetv2(train_loss_sup_1, train_loss_sup_2, train_loss_unsup, train_loss, num_batches, print_num, print_num_minus)
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(dataloaders['val']):
                    ...
                    inputs_val_4 = torch.zeros_like(inputs_val_1, device='cpu')
                    ...
                    inputs_val_4 = Variable(inputs_val_4.cuda())
                    inputs_val_1 = torch.cat([inputs_val_1, inputs_val_4], dim=1)
                    mask_val = Variable(data['mask'][tio.DATA].squeeze(1).long().cuda())
                    torch.cuda.empty_cache()

                    optimizer1.zero_grad()
                    outputs_val_1, outputs_val_2 = model1(inputs_val_2, inputs_val_3, inputs_val_1)
                    torch.cuda.empty_cache()
                ...
                if rank == args.rank_index:
                    ...
                    val_eval_list_2, val_m_jc_2 = print_val_eval_sup(cfg['NUM_CLASSES'], score_list_val_2, mask_list_val, print_num_minus)
                    best_val_eval_list1 = save_val_best_sup_3d(best_val_eval_list1, model1, val_eval_list_1, path_trained_models, 'XNetv2')
                    best_val_eval_list2 = save_val_best_sup_3d(best_val_eval_list2, model1, val_eval_list_2, path_trained_models, 'XNetv2')
                    torch.cuda.empty_cache()
