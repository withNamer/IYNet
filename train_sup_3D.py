...
if __name__ == '__main__':
    ...
    parser.add_argument('--wavelet_type', default='bior1.5', help='haar, db2, bior1.5, bior2.4, coif1, dmey')
    parser.add_argument('--train_alpha', default=[0.0, 0.4])
    parser.add_argument('--train_beta', default=[0.6, 0.8])
    parser.add_argument('--train_gamma', default=[0.6, 0.8])
    parser.add_argument('--val_alpha', default=[0.2, 0.2])
    parser.add_argument('--val_beta', default=[0.7, 0.7])
    ...
    for epoch in range(args.num_epochs):
        ...
        count_iter += 1
        if (count_iter-1) % args.display_iter == 0: # and epoch > 0
        ...
        for i, data in enumerate(dataloaders['train']):
            ...
            optimizer.zero_grad()
            pred_train_sup1, pred_train_sup2 = model(img_train_sup_2, img_train_sup_3, img_train_sup_1)
            torch.cuda.empty_cache()

            if count_iter % args.display_iter == 0: #  and epoch > 0
                if i == 0:
            ...
            outputs_train_softmax1 = torch.softmax(pred_train_sup1, dim=1)
            outputs_train_softmax2 = torch.softmax(pred_train_sup2, dim=1)
            # max_train3 = torch.max(pred_train_sup3, dim=1)[1].long()

            loss_train_sup1 = criterion(pred_train_sup1, mask_train) + F.cross_entropy(outputs_train_softmax1, mask_train)
            loss_train_sup2 = criterion(pred_train_sup2, mask_train) + F.cross_entropy(outputs_train_softmax2, mask_train)
        ...
        if count_iter % args.display_iter == 0: # and epoch > 0
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(dataloaders['val']):
                    ...
                    optimizer.zero_grad()
                    outputs_val_1, outputs_val_2= model(inputs_val_2, inputs_val_3, inputs_val_1)
                    torch.cuda.empty_cache()
                ...
                if rank == args.rank_index:
                    ...
                    best_val_eval_list1 = save_val_best_sup_3d(best_val_eval_list1, model, val_eval_list_1, path_trained_models, 'MF')
                    best_val_eval_list2 = save_val_best_sup_3d(best_val_eval_list2, model, val_eval_list_2, path_trained_models, 'MF')
                    torch.cuda.empty_cache()
