...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', default='EPFL', help='CREMI, GlaS, ISIC-2017')
    ...
    parser.add_argument('--train_scale', default=[0.6, 0.8])
    ...
    parser.add_argument('--val_scale', default=[0.8, 0.8])

    parser.add_argument('-i', '--display_iter', default=5, type=int)
    parser.add_argument('-n', '--network', default='MFwavelet', type=str)
    ...
    dataloaders = dict()
    dataloaders['train_sup'] = DataLoader(dataset_train_sup, batch_size=args.batch_size, shuffle=False, pin_memory=True, num_workers=4, sampler=train_sampler_sup)
    dataloaders['train_unsup'] = DataLoader(dataset_train_unsup, batch_size=args.batch_size, shuffle=False, pin_memory=True, num_workers=4, sampler=train_sampler_unsup)
    dataloaders['val'] = DataLoader(dataset_val, batch_size=args.batch_size, shuffle=False, pin_memory=True, num_workers=4, sampler=val_sampler)
    ...
    best_model = model1
    best_result = 'Result1'
    best_val_eval_list1 = [0 for i in range(4)]
    best_val_eval_list2 = [0 for i in range(4)]
    ...
    for epoch in range(args.num_epochs):
        ...
        for i in range(num_batches['train_sup']):
            ...
            img_train_unsup1 = Variable(torch.cat((unsup_index['image'], unsup_index['image']),dim=1).cuda())
            # img_train_unsup1 = Variable(unsup_index['image'].cuda())
            # img_train_unsup1_ = Variable(unsup_index['LH_'].cuda())
            img_train_unsup2 = Variable(unsup_index['L'].cuda())
            img_train_unsup3 = Variable(unsup_index['H'].cuda())
            optimizer1.zero_grad()
            pred_train_unsup1, pred_train_unsup2 = model1(img_train_unsup3, img_train_unsup2, img_train_unsup1)

            # max_train1 = torch.max(pred_train_unsup1, dim=1)[1].long()
            # max_train2 = torch.max(pred_train_unsup2, dim=1)[1].long()
            T = 1.0
            outputs_train_softmax_unsup1 = torch.softmax(pred_train_unsup1 / T, dim=1)
            outputs_train_softmax_unsup2 = torch.softmax(pred_train_unsup2 / T, dim=1)
            # max_train3 = torch.max(pred_train_unsup3, dim=1)[1].long()
            # 获取最大值和对应的索引（类别预测）
            logits1, max_train1 = torch.max(outputs_train_softmax_unsup1.detach(), dim=1)
            logits2, max_train2 = torch.max(outputs_train_softmax_unsup2.detach(), dim=1)

            # 创建mask（置信度>0.95的样本）
            high_confidence = 0.95
            mask1 = (logits1 > high_confidence)
            mask2 = (logits2 > high_confidence)

            # 创建ignore标签（低置信度的样本标签设为-1）
            max_train1_ignore = max_train1.masked_fill(~mask1, -1)
            max_train2_ignore = max_train2.masked_fill(~mask2, -1)

            # 计算损失
            loss_ce1 = (mask2 * F.cross_entropy(outputs_train_softmax_unsup1, max_train2, reduction='none')).sum() / (mask2.sum() + 1e-8)
            loss_ce2 = (mask1 * F.cross_entropy(outputs_train_softmax_unsup2, max_train1, reduction='none')).sum() / (mask1.sum() + 1e-8)
            loss_ignore1 = criterion(pred_train_unsup1, max_train2_ignore)
            loss_ignore2 = criterion(pred_train_unsup2, max_train1_ignore)

            loss_train_unsup = loss_ce1 + loss_ce2 + loss_ignore1 + loss_ignore2
            ...
            img_train_sup1 = Variable(torch.cat((sup_index['image'], sup_index['image']), dim=1).cuda())
            # img_train_sup1 = Variable(sup_index['image'].cuda())
            # img_train_sup1_ = Variable(sup_index['LH_'].cuda())
            ...
            pred_train_sup1, pred_train_sup2 = model1(img_train_sup3, img_train_sup2, img_train_sup1)
            ...
            outputs_train_softmax1 = torch.softmax(pred_train_sup1 / T, dim=1)
            outputs_train_softmax2 = torch.softmax(pred_train_sup2 / T, dim=1)
            loss_train_sup1 = criterion(pred_train_sup1, mask_train_sup) + F.cross_entropy(outputs_train_softmax1, mask_train_sup)
            loss_train_sup2 = criterion(pred_train_sup2, mask_train_sup) + F.cross_entropy(outputs_train_softmax2, mask_train_sup)
        ...
        if count_iter % args.display_iter == 0:
            ...
            if rank == args.rank_index:
                ...
                train_epoch_loss_sup1, train_epoch_loss_sup2, train_epoch_loss_unsup, train_epoch_loss = print_train_loss_XNetv2(train_loss_sup_1, train_loss_sup_2, train_loss_unsup, train_loss, num_batches, print_num, print_num_minus)
                train_eval_list1, train_m_jc1 = print_train_eval_sup(cfg['NUM_CLASSES'], score_list_train1, mask_list_train, print_num_minus)
                train_eval_list2, train_m_jc2 = print_train_eval_sup(cfg['NUM_CLASSES'], score_list_train2, mask_list_train, print_num_minus)
                torch.cuda.empty_cache()
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(dataloaders['val']):
                    ...
                    inputs_val1 = Variable(torch.cat((data['image'], data['image']), dim=1).cuda())
                    # inputs_val1 = Variable(data['image'].cuda())
                    # inputs_val1_ = Variable(data['LH_'].cuda())
                    ...
                    optimizer1.zero_grad()
                    outputs_val1, outputs_val2 = model1(inputs_val3, inputs_val2, inputs_val1)
                    torch.cuda.empty_cache()
                ...
                if rank == args.rank_index:
                    ...
                    val_eval_list2, val_m_jc2 = print_val_eval_sup(cfg['NUM_CLASSES'], score_list_val2, mask_list_val, print_num_minus)
                    best_val_eval_list1 = save_val_best_sup_2d(cfg['NUM_CLASSES'], best_val_eval_list1, model1, score_list_val1, name_list_val, val_eval_list1, path_trained_models, path_seg_results, cfg['PALETTE'], 'XNetv2')
                    best_val_eval_list2 = save_val_best_sup_2d(cfg['NUM_CLASSES'], best_val_eval_list2, model1, score_list_val2, name_list_val, val_eval_list2, path_trained_models, path_seg_results, cfg['PALETTE'], 'XNetv2')
                    dist.barrier()
                    torch.cuda.empty_cache()
    ...
    if rank == args.rank_index:
        ...
        print('-' * print_num)
        print_best_sup(cfg['NUM_CLASSES'], best_val_eval_list1, print_num_minus)
        print_best_sup(cfg['NUM_CLASSES'], best_val_eval_list2, print_num_minus)
        print('=' * print_num)
