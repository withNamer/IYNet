...
class MFwaveNet2(nn.Module):
    def __init__(self, in_channels=3, num_classes=2):
        ...
        self.RS_Up5_top = up_conv(ch_in=512, ch_out=256)
        ...
        self.M_Up5_top = up_conv(ch_in=1024, ch_out=512)
        ...
        self.RS_top = conv_block(ch_in=512, ch_out=512)

        self.top_fusion_RS = nn.Sequential(
            nn.Conv2d(1024 * 2 + 512, 512, kernel_size=1, stride=1, padding=0, bias=True),
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True)
        )
        self.top_fusion_M = nn.Sequential(
            nn.Conv2d(1024 * 1 + 512 * 2, 1024, kernel_size=1, stride=1, padding=0, bias=True),
            nn.BatchNorm2d(1024),
            nn.ReLU(inplace=True)
        )
        self.top_fusion_RS_UP = nn.Sequential(
            nn.Conv2d(512 * 2 + 256, 256, kernel_size=1, stride=1, padding=0, bias=True),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True)
        )
        self.top_fusion_M_UP = nn.Sequential(
            nn.Conv2d(512 * 1 + 256 * 2, 512, kernel_size=1, stride=1, padding=0, bias=True),
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x_R, x_S, x_M):
        ...
        RS_x5_top = self.RS_top(RS_x5)
        RS_d5_top = self.RS_Up5_top(RS_x5)

        M_x5_top = self.M_top(M_x5)
        M_d5_top = self.M_Up5_top(M_x5)  # 本层的上采样

        RS_d5 = self.top_fusion_RS(torch.cat((RS_x5, M_x5, M_x5_top), dim=1))
        M_d5 = self.top_fusion_M(torch.cat((M_x5, RS_x5, RS_x5_top), dim=1))

        RS_x4_fusion = self.top_fusion_RS_UP(torch.cat((RS_x4, M_d5_top), dim=1))
        M_x4_fusion = self.top_fusion_M_UP(torch.cat((M_x4, RS_d5_top), dim=1))
        ...
        RS_d4 = torch.cat((R_x3, S_x3, RS_d4, RS_x3, M_x3), dim=1)
        RS_d4 = self.RS_Up_conv4(RS_d4)

        RS_d3 = self.RS_Up3(RS_d4)
        RS_d3 = torch.cat((R_x2, S_x2, RS_d3, RS_x2, M_x2), dim=1)
        RS_d3 = self.RS_Up_conv3(RS_d3)

        RS_d2 = self.RS_Up2(RS_d3)
        RS_d2 = torch.cat((R_x1, S_x1, RS_d2, RS_x1, M_x1), dim=1)
        ...
        M_d5 = self.M_Up5(M_d5)
        RSM_d5 = torch.cat((M_x4_fusion, M_d5, RS_x4), dim=1) 
